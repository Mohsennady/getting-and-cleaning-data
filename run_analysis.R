...
test<-read_table("./testdata/UCI HAR Dataset/test/X_test.txt",col_names = FALSE)
labels<-read_lines("./testdata/UCI HAR Dataset/features.txt",n_max = -1)
names(test)<-labels
testlabels<-read_table("./testdata/UCI HAR Dataset/test/y_test.txt",
                       col_names = FALSE)
...
subjecttest<-read_table("./testdata/UCI HAR Dataset/test/subject_test.txt",
                        col_names = FALSE) %>% rename (subject=X1)
finaltest<-cbind(subjecttest,activitytest,test)
train<-read_table("./testdata/UCI HAR Dataset/train/X_train.txt",col_names = FALSE)
names(train)<-labels

trainlabels<-read_table("./testdata/UCI HAR Dataset/train/y_train.txt",
                        col_names = FALSE)
activitytrain<-trainlabels%>% mutate(activity = cut (X1, breaks = 
                 c(0,1,2,3,4,5,6),
                 labels = c("walking","walking_upstairs","walking_downstairs","sitting",
                 "standing","laying"))) %>% select(activity)
subjecttrain<-read_table("./testdata/UCI HAR Dataset/test/subject_test.txt",
                        col_names = FALSE) %>% rename (subject=X1)
